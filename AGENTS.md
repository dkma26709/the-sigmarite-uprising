...
- `build.ps1` — compiles every scenario to `pdfs/` locally (requires the Typst CLI)
...
Session work is developed on a branch named `session_<n>`. Every scenario gets a `manifest.json` entry with `id`, `name`, `typ`, `pdf` (`pdfs/<basename>.pdf`), `report`, `map`, `status` (`planned` or `completed`) and `result`. The site shows the Battle Report button only once `status` is `completed`.
...
Rules start on page 2 with these sections, in this order, **all optional** (a scenario may legitimately lack any of them, including Victory Conditions):
...
Install Typst (e.g. `winget install --id Typst.Typst`), then run `.\build.ps1` from the repo root. Serve the folder (e.g. `python -m http.server`) to preview the site with the compiled PDFs. Editing `.typ` files with encoding-unaware tools can corrupt UTF-8 characters like "·"; use an editor or tool that preserves UTF-8.
